...
def getAllLinks(startPage, domain, browserType):
    ...
    avoid = ['.pdf', '.exe', '#','jpg','.jpeg','.gif','.png']
...
def downloadAllFilesWget(list):
    ...
    prepareJson("downloads/pdfurl.txt")

    for url in list:
        parsedUrl = urlparse(url)
        fileName = str(parsedUrl.path).replace('/',"-").replace('.',"_")
        wget.download(url,'downloads/' + fileName)
        # write file : url to file
        addToJson("downloads/pdfurl.txt", url, fileName)
...
def downloadAllFilesRetrieve(list):
    ...
    for url in list:
        parsedUrl = urlparse(url)
        fileName = str(parsedUrl.path).replace('/',"-")
...
def downloadAllFilesRequestGet(list):
    ...
    #open json to grab list of urls (to do)
    data = getFromJson("downloads/pdfurl.txt")
...
#grabs all navigatable links within domain
